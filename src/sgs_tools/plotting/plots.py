...
# Plotting
def tensor_plot(tens, grid_dims=None, robust=True, **plot_kwargs):
...
def comparison_profile_plot(
    ...
    """datasets :: iterable of Xarray datasets,
      containing the plot_field as a 4-d DataArray
    reduce_coords :: iterable of dimesions to be collapsed (mean, std, etc.)
    take_slice :: dictionary of {'coord_name' : coord_index}
      to be sliced from each dataset
    plot_coord: string, name of remaining coordinated to be plotted against.
    plot_field: string, name of field to be plotted,
      must be present in all datasets
    height_range: optional, cutoff y-axis,
      any format accepted by the [height_range] syntax
    axes: optional, an iterable of matplotlib Axis objects to plot on,
      if absent, create own figure
    returns figure with 2 panels: comparing mean and std profiles
    ...
    ls = ["solid", "dashed", "dotted"] if ls is None else len(colors) * ls
