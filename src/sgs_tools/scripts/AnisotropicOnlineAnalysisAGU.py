...
def parse_args(arguments: Sequence[str] | None = None) -> dict[str, Any]:
    parser = ArgumentParser(
        description="""Analyze anisotropic SGS model coefficients by comparing
                    diagonal, isotropic, and Smagorinsky CS models.
                    Creates diagnostic plots for visualizing anisotropy effects.
                """,
    ...
    fname.add_argument(
        "ds_diag",
        type=Path,
        help=""" Location of diagonal CS simulation outputs --
            UM NetCDF diagnostic files. Recognizes glob patterns
            and walks directory trees, e.g. './my_file_p[br]*nc'.
            Can have multiple files, but only one glob pattern.
            All files in a glob should belong to same simulation.
        """,
    )
    ...
    fname.add_argument(
        "ds_iso",
        type=Path,
        help=""" Location of isotropic CS simulation outputs --
            UM NetCDF diagnostic files. Recognizes glob patterns
            and walks directory trees, e.g. './my_file_p[br]*nc'.
            Can have multiple files, but only one glob pattern.
            All files in a glob should belong to same simulation.
    ...
    fname.add_argument(
        "ds_smag",
        type=Path,
        help=""" Location of default Smagorinsky simulation outputs
            -- UM NetCDF diagnostic files. Recognizes glob patterns
            and walks directory trees, e.g. './my_file_p[br]*nc'.
            Can have multiple files, but only one glob pattern.
            All files in a glob should belong to same simulation.
    ...
    fname.add_argument(
        ...
        type=float,
        nargs="+",
        help="""Horizontal resolution (will overwrite coordinates).
            If single resolution given, applies to all input files.
            Else, give as many resolutions as input_file patterns.
            **NB** works for ideal simulations only.
        """,
    )
    ...
    fname.add_argument(
        ...
        nargs="*",
        default=[],
        help="""Times at which to perform analysis (code coords).
            Will find nearest available match. Empty (default)
            means full data range at 1h intervals.
        """,
    )
    ...
    fname.add_argument(
        ...
        nargs=2,
        default=[-1, -1],
        help="""Vertical interval in code coordinates.
            Negative values interpreted as take min/max.
        """,
    )
    ...
    plotting.add_argument(
        ...
        nargs="*",
        default=[],
        help="""Vertical heights for horizontal slice plots.
            If not given, these plots will be omitted.
        """,
    )
    ...
    # Collated input files. Order must match plotting_styles.
    args["input_files"] = [args["ds_diag"], args["ds_iso"], args["ds_smag"]]

    # check input args
    if len(args["h_resolution"]) == 1:
        res = args["h_resolution"][0]
        args["h_resolution"] = [res] * len(args["input_files"])
    else:
    ...
    if args["hor_slice_levels"]:
        assert all(
            ...
            f"hor_slice_levels {args['hor_slice_levels']} "
            f"not in z_range {args['z_range']}"
        )
    return args
...
def add_offline_fields(
    ...
    # NOTE: these rely on ds pre-interpolated to single grid
    # (no staggering)
    ...
    # # diagonal cs components
    # try:
    #   cs_list = [ds[f"cs_1"], ds[f"cs_2"], ds[f"cs_3"]]
    #   ds["cs_diag"] = xr.DataArray(
    #       cs_list, dims='c1', coords={'c1': [1, 2, 3]}
    #   )
    # except KeyError:
    #     print("Can't add cs_diag with missing ingredients")

    # # diagonal theta cs components
    # try:
    #   cs_theta_list = [
    #       ds[f"cs_theta_1"], ds[f"cs_theta_2"], ds[f"cs_theta_3"]
    #   ]
    #   ds["cs_theta_diag"] = xr.DataArray(
    #       cs_theta_list, dims='c1', coords={'c1': [1, 2, 3]}
    #   )
    # except KeyError:
    #     print("Can't add cs_theta_diag with missing ingredients")
...
def preprocess_dataset(
    ...
    # take z-range
    z_coords = ("z", "z_rho", "z_theta", "z_face", "z_centre", "thlev_bl_zsea_theta")
    for z in z_coords:
        if z in ds:
...
def plot1(
    ...
    ds_iso: xr.Dataset,
    ds_smag: xr.Dataset,
    plot_map: dict[str, field_plot_kwargs],
) -> Figure:
    # data reduction
    da_collection = {}
    linecolor = {}
    ...
    # Backend-safe figure creation
    fig = Figure(figsize=(6 * len(times), 4))
    FigureCanvasAgg(fig)
    axes = fig.subplots(1, len(times), sharey=False)
    if len(times) == 1:
...
def plot(
    ds_collection: Mapping[str, xr.Dataset],
    args: dict[str, Any],
    field_plot_map: dict[str, field_plot_kwargs],
) -> None:
    ...
    with timer("Plot 1 profiles", "s"):
        try:
            ...
            render_figure(
                plot1_fig,
                path=args["plot_path"],
                filename=f"Profile_{tlabel}_mean_Cs_comparison.png",
                show=args["plot_show"],
            )
        except KeyboardInterrupt:
            print("Detected Keyboard interrupt, proceeding with analysis")
...
def main(arguments: Sequence[str] | None = None) -> None:
    ...
    with timer("Total execution time", "min"):
        with timer("Arguments", "ms"):
            ...
            print_header("anisotropic_analysis")
