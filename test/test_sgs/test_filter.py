...
def test_coarse_grain_filter():
    ...
    field = xr.DataArray(
        field_data, dims=["x", "y"], coords={"x": np.arange(4), "y": np.arange(4)}
    )

    # Create a sample  kernels
    kernel_2d = xr.DataArray(np.array([[0.25, 0.25], [0.25, 0.25]]), dims=["x", "y"])

    kernel_deg = xr.DataArray(np.array([[0.5, 0.5]]), dims=["x", "y"])

    kernel_1d = xr.DataArray(np.array([0.5, 0.5]), dims=["x"])
    expected_2d = np.array([[3.5, 5.5], [11.5, 13.5]])
    expected_1d = np.array([[3, 11], [4, 12], [5, 13], [6, 14]])

    for kernel, expected_data in zip(
        [kernel_2d, kernel_deg, kernel_1d], [expected_2d, expected_1d, expected_1d]
    ):
...
def test_coarse_grain_invalid_dims():
    ...
    kernel = xr.DataArray(np.array([[0.25, 0.25], [0.25, 0.25]]), dims=["x", "y"])
